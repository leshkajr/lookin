        ...
        <div class="d-flex flex-wrap align-items-center justify-content-left listings">
            @foreach($listings as $listing)
                <div class="listing-block">
                    ...
                    <div class="m-2 mt-3">
                        <div class="row">
                            <div class="col listing-localization">{{ $listing['city']['name'] }}, {{ $listing['country']['name'] }}</div>
                            <div class="col text-end">
                                ...
                                <span style="margin-left: 5px;"> {{ $listing['rating'] }}</span>
                            </div>
                        </div>
                        <div class="listing-category">{{ Lang::get('categories.'.$listing['category']->nameCategoryListing) }}</div>
                        <div class="listing-date">
                            @if($searchInfo['arrivalDateMonth'] === $searchInfo['departureDateMonth'])
                                {{ $searchInfo['arrivalDateDay'] }}-{{ $searchInfo['departureDateDay'] }}
                                {{ $searchInfo['arrivalDateMonth'] }}.
                            @else
                                {{ $searchInfo['arrivalDateDay'] }} {{ $searchInfo['arrivalDateMonth'] }}.
                                -
                                {{ $searchInfo['departureDateDay'] }} {{ $searchInfo['departureDateDay'] }}.
                            @endif
                            </div>
                        <div class="listing-price-block"><span class="listing-price">{{ $listing['priceForNight'] }}$</span> for night</div>
                    </div>
                </div>
            @endforeach
...
@section('scripts')
    ...
    <script src="{{ URL::asset('js/routeToRoom.js')}}"></script>
    <script>
        startCalendar('{{ App::getLocale() }}');
        firstShowLocation();
        @if(isset($_GET['location_id']))
            chooseLocation({{ $location['city']->id }},'{{ $location['city']->name }}','{{ $location['country']->name }}','{{ $_GET['location_type'] }}');
        @endif
    </script>
@stop
